...
static struct symbol_table_t {
    char func_name[256];
    void (*server_func)(rmc_node_id_t node_id, uint32_t trans_id, uint8_t*);
...
//
static void (*dstc_find_local_function(char* name, int name_len))(rmc_node_id_t node_id, uint32_t trans_id, uint8_t*)
{
    ...
    return (void (*) (rmc_node_id_t, uint32_t, uint8_t*)) 0;
}
...
static uint32_t dstc_process_function_call(uint8_t* data, uint32_t data_len)
{
    ...
    void (*local_func_ptr)(rmc_node_id_t node_id, uint32_t trans_id, uint8_t*) = 0;
    ...
    RMC_LOG_DEBUG("DSTC Serve: node_id[%lu] trans_id[%lu] name_len[%d] name[%.*s] payload_len[%d]",
                  call->node_id, call->trans_id,
                  call->name_len,
    ...
    (*local_func_ptr)(call->node_id, call->trans_id, call->payload + call->name_len); 
    return sizeof(dstc_header_t) + call->payload_len;
}
...
void _dstc_queue(uint8_t* name, uint8_t* arg, uint32_t arg_sz)
{
    ...
    call->name_len = strlen(name);
    call->payload_len = arg_sz + call->name_len;
    call->node_id = dstc_get_node_id();
    call->trans_id = trans_id++;
    memcpy(call->payload, name, call->name_len);
    memcpy(call->payload + call->name_len, arg, arg_sz);

    RMC_LOG_DEBUG("DSTC Queue: node_id[%lu] trans_id[%lu] name_len[%d] name[%.*s] payload_len[%d]",
                  call->node_id, call->trans_id,
                  call->name_len,
                  call->name_len, call->payload, call->payload_len - call->name_len);
    rmc_pub_queue_packet(&_dstc_pub_ctx, call, sizeof(dstc_header_t) + call->name_len + arg_sz, 0);
    return;
}
