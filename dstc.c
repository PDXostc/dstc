...
//
static void (*dstc_find_server_function(char* name, int name_len))(rmc_node_id_t node_id,
                                                                   uint8_t* func_name,
                                                                   ...
                                                                   uint8_t* payload)
{
    ...
    return (void (*) (rmc_node_id_t, uint8_t*, uint8_t, uint8_t*)) 0;
}
...
//
void dstc_register_server_function(char* name, void (*server_func)(rmc_node_id_t node_id,
                                                                   uint8_t* func_name,
                                                                   uint8_t func_name_len,
                                                                   uint8_t*))
{
...
// dstc_register_server_function()
//
static void (*dstc_find_callback(dstc_callback_int_t func_addr))(rmc_node_id_t node_id,
                                                                 uint8_t* func_name,
                                                                 uint8_t func_name_len,
                                                                 uint8_t* payload)
{
...
//
void dstc_register_callback_server(void (*callback)(rmc_node_id_t node_id,
                                                    uint8_t* func_name,
                                                    uint8_t func_name_len,
                                                    uint8_t*))
{
                                           ...
                                           uint32_t data_len)
{
    ...
    dstc_internal_callback_t local_func_ptr = 0;
    ...
    (*local_func_ptr)(call->node_id,
                      ...
                      call->name_len, // Length of name
                      call->payload + (call->name_len?call->name_len:sizeof(uint64_t)));
    return sizeof(dstc_header_t) + call->payload_len;
}
...
static int dstc_queue(uint8_t* name, uint8_t name_len, uint8_t* arg, uint32_t arg_sz)
{
    ...
    // If we have pending calls in the DSTC circular buffer, try to queue them with RMC.
    // This may fail if we are currently suspended from sending traffic over RMC due
    // to congestion.
